...
CREATE VIEW ProductosOrdenados AS
	SELECT Producto, IDOrden
		FROM Productos JOIN OrdenesdeVenta ON (ProductidOrdVent = Productid) 
        WHERE Producto = Productos.Nombre AND IDOrden = OrdenVentaid;


CREATE VIEW ZonasOrdenes AS
	SELECT CodigoPostalZona,Ordenes 
		FROM Direcciones JOIN OrdenesdeVenta ON (Direccoin.Direccionid = OrdenesdeVenta.DireccionOrdVent) 
        WHERE CodigoPostalZona = MAX(COUNT(Direccion.CodigoPostal)); 
	

CREATE UNIQUE INDEX I_BikeWorld_NIT
    ON BikeWorld(NIT);
    
CREATE UNIQUE INDEX I_Producto_Productid
